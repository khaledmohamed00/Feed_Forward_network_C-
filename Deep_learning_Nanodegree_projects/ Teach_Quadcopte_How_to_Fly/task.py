...
class Task():
    ...
    def __init__(self, init_pose=None, init_velocities=None, 
        ...
        self.state_size = self.action_repeat *6 #(6 + 3 + 3)
        self.action_low = 0
    ...
    def get_reward(self):
        ...
        #distance=0
        #dist=np.sqrt(((self.sim.pose[:3]-self.target_pos)**2).sum())
        #vel=np.sqrt(self.sim.v[0]**2+self.sim.v[1]**2+self.sim.v[2]**2)
        #dist_reward=1-(dist)**0.4
        reward = np.tanh(1.-.0003*(abs(self.sim.pose[:3] - self.target_pos)).sum())
        #vel_discount=((1-max(vel,0.1)))**(1/max(dist,0.1))
        
        #reward= vel_discount*dist_reward-0.001*(self.sim.pose[:3]).sum()#+0.001*(self.sim.pose[2])                   
        '''
        reward = 0
        penalty = 0
        ...
        penalty += 1000*abs(current_position[2]-self.target_pos[2])**2
